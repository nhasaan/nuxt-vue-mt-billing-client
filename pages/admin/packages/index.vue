<template>
  <div class="container-fluid">
    <div class="text-center">
      <h3 class="title">
        Package datatable using vuetify
      </h3>
      <hr>
      <div v-if="datas.length === 0">
        <h2>No package found at the moment</h2>
      </div>
    </div>
                ...
                <v-card>
                  ...
                  <v-card-text>
                    <v-container>
                      <v-row>
                        <v-col cols="12">
                          <v-text-field v-model="editedItem.name" label="Package name" />
                        </v-col>
                        <v-col cols="12">
...
export default {
  ...
  data: () => ({
    ...
    selected: [],
    headers: [
      { text: 'Package Name', align: 'left', sortable: true, value: 'name' },
  ...
  computed: {
    formTitle () {
      return this.editedIndex === -1 ? 'Create Package' : 'Edit Package'
    },
    url () {
      return `${process.env.API_BASE_URL}/packages?page=${this.page}&limit=${this.itemsPerPage}`
    }
  },
  watch: {
    dialog (val) {
  ...
  methods: {
    ...
    deleteItem (item) {
      ...
      if (confirm('Are you sure you want to delete this package?')) {
        axios
          .delete(`${process.env.API_BASE_URL}/packages/${item._id}`)
          .then((data) => {
            this.datas.splice(index, 1)
            this.$toast.show(
              "Success! '" + item.title + "' package deleted...",
              {
    ...
    save () {
      if (this.editedIndex > -1) {
        ...
        axios
          .put(`${process.env.API_BASE_URL}/packages/${payload._id}`, payload)
          .then((data) => {
            // console.log(data);
            this.$toast.show(
              "Success! Package '" + payload.title + "' updated...",
              {
                position: 'bottom-center',
                duration: 3000
              }
        ...
        const payload = this.editedItem
        axios
          .post(`${process.env.API_BASE_URL}/packages`, payload)
          .then((data) => {
            // console.log(data);
            this.products.push(this.editedItem)
            this.$toast.show(
              "Success! New package '" + payload.title + "' created.",
              {
                position: 'bottom-center',
                duration: 3000
              }
