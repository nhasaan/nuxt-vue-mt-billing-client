<template>
  <div class="container-fluid">
    <div class="text-center">
      <h3 class="title">
        Client datatable using vuetify
      </h3>
      <hr>
        ...
        >
          ...
          <template v-slot:top>
            <v-toolbar flat color="white">
              <v-toolbar-title>Client CRUD</v-toolbar-title>
                ...
                <v-card>
                  ...
                  <v-card-text>
                    <v-container>
                      <v-row>
                        <v-col cols="12">
                          <v-text-field v-model="editedItem.name" label="Client name" />
                        </v-col>
                        <v-col cols="12">
...
export default {
  ...
  data: () => ({
    ...
    selected: [],
    headers: [
      { text: 'Client Name', align: 'left', sortable: true, value: 'name' },
  ...
  computed: {
    ...
    url () {
      return `${process.env.API_BASE_URL}/clients?page=${this.page}&limit=${this.itemsPerPage}`
    }
  },
  watch: {
    dialog (val) {
  ...
  methods: {
    ...
    deleteItem (item) {
      ...
      if (confirm('Are you sure you want to delete this client?')) {
        axios
          .delete(`${process.env.API_BASE_URL}/clients/${item._id}`)
          .then((data) => {
            this.datas.splice(index, 1)
            this.$toast.show(
              "Success! '" + item.title + "' client deleted...",
              {
    ...
    save () {
      if (this.editedIndex > -1) {
        ...
        axios
          .put(`${process.env.API_BASE_URL}/clients/${payload._id}`, payload)
          .then((data) => {
            // console.log(data);
            this.$toast.show(
              "Success! Client '" + payload.title + "' updated...",
              {
                position: 'bottom-center',
                duration: 3000
              }
        ...
        const payload = this.editedItem
        axios
          .post(`${process.env.API_BASE_URL}/clients`, payload)
          .then((data) => {
            // console.log(data);
            this.products.push(this.editedItem)
            this.$toast.show(
              "Success! New client '" + payload.title + "' created.",
              {
                position: 'bottom-center',
                duration: 3000
              }
